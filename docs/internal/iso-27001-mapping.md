...
Inner Warden maps to 12 ISO 27001:2022 Annex A controls through its automated detection, response, and audit capabilities. The mapping is computed at runtime by the `/api/compliance` endpoint and displayed in the dashboard Compliance tab, reflecting the current configuration state.
...
| A.8.1 | Asset management | Sensor inventory tracking 15 collectors (auth_log, journald, docker, nginx x2, exec_audit, eBPF, suricata, wazuh, osquery, syslog_firewall, firmware_integrity, cloudtrail, macos_log, falco_log) | Always met | `/api/sensors` endpoint; Sensors HUD in dashboard |
...
| A.12.4 | Logging and monitoring | 39+ stateful detectors, 22 eBPF kernel hooks, continuous collection from all enabled sources | Always met | `events-YYYY-MM-DD.jsonl`, `incidents-YYYY-MM-DD.jsonl`; `/api/sensors` |
| A.12.6 | Technical vulnerability management | Execution guard performs shell command AST analysis via tree-sitter-bash to detect exploit payloads | Met when `execution_guard_enabled = true` | Incidents with detector `execution_guard`; `/api/agent/check-command` endpoint |
| A.13.1 | Network security management | Automated IP blocking at multiple layers: XDP (wire-speed), iptables/ufw/nftables/pf, Cloudflare WAF | Met when responder is enabled and not in dry-run mode (`enabled = true`, `dry_run = false`) | Decision entries with `action = "block-ip"`; XDP blocklist at `/sys/fs/bpf/innerwarden/blocklist` |
| A.16.1 | Incident management | Full pipeline: detection (39+ detectors) -> correlation (cross-detector confidence boost) -> AI triage -> automated response -> notification (Telegram, Slack, webhook, Web Push) | Always met | `incidents-YYYY-MM-DD.jsonl`; `decisions-YYYY-MM-DD.jsonl`; notification logs |
...
3. **ISO 27001 Controls** - score in the format `N / 12` (controls met out of total)
...
3. **ISO 27001 Control Mapping** - table of 12 controls with met/not-met status and the specific reason or required action
...
1. **Control status is dynamic.** The 12 controls reflect the current agent configuration. An auditor should verify the running configuration via `GET /api/compliance` or the dashboard Compliance tab rather than relying on static documentation.
